...
class TaskItem(TwoLineAvatarIconListItem):
    ...
    def checked(self, check, list_item):
        """відмічаємо виконане завдання"""
        if check.active == True:
            app.db.put(self.key, task = self.text, 
                       date = self.secondary_text, done = True)
        else:
            app.db.put(self.key, task = self.text, 
                       date = self.secondary_text, done = False)
...
class NewTaskScreen(Screen):
    ...
    def back(self):
        ...
        self.manager.transition.duration = 0.7
        self.manager.current = "main_screen"
    
    def show_date_picker(self):
        #відкриваємо вікно вибору дати
...
class ToDoApp(MDApp):
    def build(self):
        ...
        Builder.load_file('todo_style.kv')

        self.screen = TaskListScreen(name="main_screen")
        sm = ScreenManager()
        sm.add_widget(self.screen)
        sm.add_widget(NewTaskScreen(name="newtask_screen"))
        self.read_tasks()
        return sm
    
    def read_tasks(self):
        '''Read tasks from json file'''
        self.db = JsonStore("tasks_db.json")
        # self.db.put(0, task = "домашка", date = "11.01.2023", done = False)
        # self.db.put(1, task = "домашка2", date = "11.01.2023", done = False)
        self.tasks = []
    ...
    def add_task(self, text, date):
        if text != "":
            ...
            self.dialog = MDDialog(
                text="Спочатку введіть, будь ласка, завдання!",
                buttons=[
